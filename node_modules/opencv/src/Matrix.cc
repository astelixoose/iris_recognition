...
Matrix::Init(Handle<Object> target) {
  ...
  NODE_SET_PROTOTYPE_METHOD(constructor, "houghLinesP", HoughLinesP);
	NODE_SET_PROTOTYPE_METHOD(constructor, "houghCircles", HoughCircles);
...
Matrix::GaussianBlur(const v8::Arguments& args) {
	...
	int sigmaX = args[1]->NumberValue();
	int sigmaY = args[2]->NumberValue();

	cv::GaussianBlur(self->mat, blurred, ksize, sigmaX, sigmaY);
...
Handle<Value>
Matrix::HoughCircles(const v8::Arguments& args) {
	HandleScope scope;

	Matrix *self = ObjectWrap::Unwrap<Matrix>(args.This());

	std::vector<cv::Vec3f> circles;
	double dp = args[0]->NumberValue();
	double minDist = args[1]->NumberValue();
	double param1 = args[2]->NumberValue();
	double param2 = args[3]->NumberValue();
	double minRadius = args[4]->NumberValue();
	double maxRadius = args[5]->NumberValue();

try {

	cv::HoughCircles(self->mat, circles, CV_HOUGH_GRADIENT, dp, minDist, param1, param2, minRadius, maxRadius);

} catch (cv::Exception &err) {
	v8::Local<v8::Array> arr1 = v8::Array::New(0);
	return scope.Close(arr1);
}
...
/**/
  v8::Local<v8::Array> arr = v8::Array::New(circles.size());

  for (unsigned int i=0; i<circles.size(); i++){
    v8::Local<v8::Array> pt = v8::Array::New(3);
    pt->Set(0, Number::New((double) circles[i][0]));
    pt->Set(1, Number::New((double) circles[i][1]));
    pt->Set(2, Number::New((double) circles[i][2]));
    arr->Set(i, pt);
  }
